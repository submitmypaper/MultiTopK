...
TreeNode* Tree::creatNewNode(long st, long ed, int kmin, int kmax) {
	TreeNode* newnode = new TreeNode;
	newnode->st = st;
	newnode->ed = ed;
	newnode->lowerbound = kmin;
	newnode->upperbound = kmin;
...
void Tree::initTreeParm(int kmin) {
	...
	splitMap.insert(pair<int, pair<int, int>>(split, make_pair(count, kmax)));
	tstruct.leaflen = enodeVec[enodeVec.size() - 1].lbound;
...
void Tree::splitElist(TStream& ts, list<pair<long, long>>& elist) {
	...
	while (elist.size() < 4)
	{
		...
		while (it != elist.end())
		{
			...
			values.push_back(eNode(st, mid - 1, ts.mink, ts.maxk));
			values.push_back(eNode(mid, it->second, ts.mink, ts.maxk));
			it++;
		}
		initTreeStruct(values, ts.mink, ts.maxk);
		values = vector<eNode>();
	}
}


void Tree::initTreeStruct(vector<eNode>& values, int kmin, int kmax) {
	...
	while (!nodeQueue.empty() && index < values.size()) {
		...
		if (index < values.size()) {
			...
			currentNode->right = creatNewNode(values[index].st, values[index].ed, kmin, kmax);
			nodeQueue.push_back(currentNode->right);
			index++;
		}

		if (index < values.size()) {
			currentNode->left = creatNewNode(values[index].st, values[index].ed, kmin, kmax);
...
void Tree::initEnodeVec(TStream& tstream) {
	...
	root = creatNewNode(0, tstream.maxn - 1, tstream.mink, tstream.maxk);
	...
	while (qit != tstream.workLoadVec.end())
	{
		while (elist.size() < 3)
		{
			...
			{
				enodeVec.push_back(eNode(etop.first, etop.second, ktem, tstream.maxk));
				elist.pop_front();
				etop = elist.front();
				if (!nodeQueue.empty())
				{
					nodeQueue.front()->lowerbound = ktem;
		...
		enodeVec.push_back(eNode(etop.first, etop.second, kmax > ktem ? kmax : ktem, tstream.maxk));
		elist.pop_front();
		etop = elist.front();
		nodeQueue.front()->lowerbound = kmax > ktem ? kmax : ktem;
		nodeQueue.front()->isleaf = 1;
		nodeQueue.pop_front();
	...
	while (!nodeQueue.empty())
	{
		nodeQueue.front()->lowerbound = ktem;
		nodeQueue.front()->isleaf = 1;
		enodeVec.push_back(eNode(nodeQueue.front()->st, nodeQueue.front()->ed, ktem, tstream.maxk));
		nodeQueue.pop_front();
	}
...
void Tree::initTreeNode() {
	...
	for (auto it = candiMap.begin(); it != candiMap.end(); )
	{
		...
		if ((*nlit)->nodelist.empty() || (*nlit)->nodelist.size() < (*nlit)->lowerbound)
		{
...
void Tree::updateBinaryTree(Object Onew, int ksum, int kmax)
{
	long st;
	...
	{
		st = lastnode->ed + 1;
		TreeNode* tnode = creatNewNode(st, st + tstruct.partition - 1, tstruct.leaflen, kmax);
		tnode->isleaf = 1;
		partHeap.clear();
		partHeap.insert(Onew);
		headNodeList.push_back(tnode);
	}
	else if (ksum > 1 && ksum <= lastnode->lowerbound)
	{
		partHeap.insert(Onew);
	}
	else
	{
		if (Onew.value > theata)
		{
			if (partHeap.size() < lastnode->upperbound)
			{
				partHeap.insert(Onew);
			}
		}
		else if (Onew.value > partHeap.getMin().value)
		{
			partHeap.insert(Onew);
			partHeap.removeMin();
		}
...
void Tree::mergeNodes(TStream& tstream, int psum) {
	...
	for (int i = 0; i < times; i++)
	{
		...
		if (i == 0)
		{
			if (psum == 2)
			{
				theata = leftn->nodelist.back().value > rightn->nodelist.back().value ? rightn->nodelist.back().value : leftn->nodelist.back().value;
			}
			else
			{
				theata_temp = leftn->nodelist.back().value > rightn->nodelist.back().value ? rightn->nodelist.back().value : leftn->nodelist.back().value;
				theata = theata > theata_temp ? theata_temp : theata;
			}

		}
		TreeNode* newnode = creatNewNode(rightn->st, leftn->ed, tstream.mink, tstream.maxk);
...
void Tree::updateDataFlow(TStream& tstream) {
	...
	for (long i = tstream.GetDataStreamBegin(); i < tstream.GetVecSize(); i++)
	{
		...
		tstream.winTag++;
		tstream.winBegain++;
		updateBinaryTree(tstream.GetDataById(i), ksum, tstream.maxk);
		if (ksum == tstruct.partition)
		{
			psum++;
			ksum = 0;
